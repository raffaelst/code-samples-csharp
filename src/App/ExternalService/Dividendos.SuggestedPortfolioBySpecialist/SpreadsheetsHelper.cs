using Dividendos.SuggestedPortfolioBySpecialist.Interface;
...
namespace Dividendos.SuggestedPortfolioBySpecialist
{
    public class SpreadsheetsHelper : ISpreadsheetsHelper
    {
        ...
        public ImportFinanceResult ReadEntries()
        {
            ...
            if (values != null && values.Count > 0)
            {
                ...
                {
                    stocks.Add(new Stock() { IdStock = row[0].ToString(), MarketPrice = row[1].ToString(), TradeTime = row[2].ToString() });
